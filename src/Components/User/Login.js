...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      // Use the API helper function for login
      ...
      toast.success(`Hi ${name}, you have successfully logged in.`);

      localStorage.setItem('toastMessage', `Hi ${name}, you have successfully logged in.`);
      ...
      <div className='w-full h-full flex justify-center items-center overflow-auto'>
        <div className='lg:w-2/6 md:w-3/6 w-[90%] h-auto min-h-[300px] my-[50px] mt-[100px] md:my-0px md:py-16 p-8 bg-[#333] flex flex-col gap-16 items-center md:hover:scale-105 duration-300 shadow-xl shadow-black'>
          ...
          <span className='w-full flex relative hover:scale-105 duration-300'>
            ...
            <span>
              <Link className='text-white font-semibold hover:text-yellow-500 duration-300' to={'/signup'}>Don't have an account?</Link>
              <br />
              <Link className='text-white font-semibold hover:text-yellow-500 duration-300' to={'/forgot-password'}>Forgot Password?</Link>
            </span>
